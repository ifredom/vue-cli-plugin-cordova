...
module.exports = (api, options) => {
  ...
  const runServe = async (platform, args) => {
    ...
    if (availablePlatforms.includes(platform)) {
      ...
      const projectDevServerOptions = options.configureWebpack.devServer || {}
      // resolve server options
      const open = false // browser does not need to be opened
      const https = options.configureWebpack.devServer.https === null || options.configureWebpack.devServer.https === undefined ? true : options.configureWebpack.devServer.https // check devServer.options for user defined https setting
